...
namespace ObApi.Controllers {
	public class ForumsController : ApiController {
		...
		public HttpResponseMessage Get(int id=0, int skip = 0, int take = 25, bool includeSticky = true) {
			...
			if (id>0) {
				...
				if (memberId.HasValue) {
					...
					if (mpm.HasValue)
						result = Forum.FindWithThreads(mpm.Value, id, true, true, skip, take);
					else
						result = Forum.FindWithThreads(MemberPermissionLevel.Unauthenticated, id, false, includeSticky, skip, take);
				}
				else {
					result = Forum.FindWithThreads(MemberPermissionLevel.Unauthenticated, id, false, includeSticky, skip, take);
				}
				...
				if (memberId.HasValue) {
					...
					if (mpl.HasValue) return Request.CreateResponse<List<Forum>>(HttpStatusCode.OK, Forum.Find(mpl.Value));
				}
